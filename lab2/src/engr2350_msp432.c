...
void init_motor_PWM(){
    ...
    TA0CCTL3 = TA0CCTL4 = OUTMOD_3;
    TA0CCR0 = ((((uint32_t)1e8)>>3)*10)/1000-1;    // Set period to 10 ms (100 Hz) (-1 as it counts past TA0CCR0)
    TA0CCR3 = TA0CCR4 = TA0CCR0*.80;  // Slow
    TA0CTL |= MC__UP;
    // Enable the pullUP resistors on BMP0/5
    P4OUT |= BIT0|BIT7;
    P4REN |= BIT0|BIT7;
}
